...
(defvar mo-font "PragmataPro Mono Liga-12"
  "Font to use.")
...
(use-package org
  ...
  (setq org-todo-keywords
        '( ( sequence "TODO(t!)" "NEXT(n@)" "PROG(p!)" "WAIT(h@)" "|"
...
(use-package rustic
  ...
  ( :keymaps 'rustic-mode-map
    ...
    "C-M-s-S-f" #'rustic-cargo-clippy-fix
    "C-M-s-f" #'rustic-cargo-clippy
  ...
  (setq eshell-prompt-function
        (lambda ()
          (concat (propertize (format-time-string "[%H:%M:%S] " (current-time))
                  ...
                  (propertize (if (or (= (user-uid) 0)
                                      (string= "sudo" (file-remote-p default-directory 'method)))
                                  " #" " λ")
