...
- **Comments**: simple one-liners placed directly above the form they describe (`;; Skip star buffers when cycling to the previous or next buffer`), and only where the intent isn't obvious from the code itself. Capitalize the first word; no trailing period. Multi-line comments are reserved for non-obvious workarounds (bug references, subtle interactions). Never mention specific Emacs versions in comments or docstrings ("new in Emacs 30", "works around an Emacs 31 change") — the config tracks the latest Emacs, so version qualifiers only go stale.
...
- **Tree-sitter modes**: `treesit-enabled-modes` remaps base modes to their tree-sitter variants, so files open in `*-ts-mode` when the grammar is available. When wiring per-language configuration (hooks, LSP whitelist, mode-local bindings, etc.), target **both** the legacy and the tree-sitter mode — e.g. `python-mode` *and* `python-ts-mode`, `c++-mode` *and* `c++-ts-mode`. See `mo-lsp-enable-for-modes` for the established pattern.
- **Theme-dependent setup**: code that adjusts faces or otherwise depends on the active theme should attach to the built-in `enable-theme-functions` hook (handlers receive the theme as an argument) rather than running at file load — load-time face setup gets silently overridden when a theme is later enabled. An `after-init` re-run in the `custom` block covers handlers registered after the theme was loaded during startup. See `mo-lsp-configure-theme` for the canonical pattern.
